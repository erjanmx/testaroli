...
func TestOverriddenNotCalled(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "function github.com/grubbydistr/testaroli.qux was not called") {
		t.Errorf("expected error")
	}
}
...
func TestOverriddenCalledOnce(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "function github.com/grubbydistr/testaroli.qux was called 1 time(s) instead of 2") {
		t.Errorf("expected error")
	}
}
